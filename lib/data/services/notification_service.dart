...
class NotificationService {
  ...
  Future<void> deleteAll() async {
    final uid = _uid;
    if (uid == null) return;
    final snap = await _notificationsCol(uid)!.get();
    final batch = _db.batch();
    for (final doc in snap.docs) {
  ...
  bool _isCategoryEnabled(Map<String, bool> prefs, NotificationCategory category) {
    switch (category) {
  ...
  static Future<void> notifyApplicationAccepted({
    ...
    final name = employerName?.trim().isNotEmpty == true ? employerName! : 'Nhà tuyển dụng';
    await svc.sendToUser(
      userId: candidateId,
      title: 'Đơn ứng tuyển được chấp nhận',
      body: '$name đã chấp nhận bạn cho vị trí "$jobTitle". Mở tin nhắn để trao đổi.',
