...
class _RegisterScreenState extends State<RegisterScreen>
  ...
  bool _validateSocialRegister() {
    ...
    if (_isEmployer) {
      if (_companyCtrl.text.trim().isEmpty) {
        _showError('Vui lòng nhập tên doanh nghiệp trước khi đăng ký');
        return false;
      }
      if (_addressCtrl.text.trim().isEmpty) {
        _showError('Vui lòng nhập địa chỉ doanh nghiệp trước khi đăng ký');
        return false;
      }
    }
    return true;
    ...
    try {
      ...
      if (error != null) {
        ...
        Navigator.pushNamed(
          ...
          arguments: {'email': _emailCtrl.text.trim(), 'role': role},
        );
      }
    } finally {
  ...
  Future<void> _handleGoogle() async {
    if (!_validateSocialRegister()) return;
    setState(() => _isLoading = true);
    try {
      final role = _isEmployer ? 'employer' : 'candidate';
      final user = await _authCtrl.registerWithGoogle(
        role: role,
        firstName: _firstNameCtrl.text.trim(),
        lastName: _lastNameCtrl.text.trim(),
        username: _usernameCtrl.text.trim(),
  ...
  Future<void> _handleFacebook() async {
    ...
    try {
      final role = _isEmployer ? 'employer' : 'candidate';
      final user = await _authCtrl.registerWithFacebook(
        role: role,
        firstName: _firstNameCtrl.text.trim(),
        lastName: _lastNameCtrl.text.trim(),
        username: _usernameCtrl.text.trim(),
        ...
        children: [
          ...
          SlideTransition(
            ...
            child: TweenAnimationBuilder<double>(
              tween: Tween(
                begin: _isEmployer ? 0 : 1,
                end: _isEmployer ? 1 : 0,
              ),
              duration: const Duration(milliseconds: 450),
              builder: (context, t, _) {
                final colors = [
                  Color.lerp(_candidateColors[0], _employerColors[0], t)!,
                  Color.lerp(_candidateColors[1], _employerColors[1], t)!,
                ];
